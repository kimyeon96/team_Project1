...
public class ConService implements IConService {
    ...
    public List<ConDTO> getSearchCon(List<String> general, String cityParam, String districtParam, String HptorClnParam, String CODE_MSTParam, String dayParam, String hosnameParam, String pageNoParam, String ordParam, String numOfRowsParam, String telParam) throws Exception {
        ...
        NodeList lat = document.getElementsByTagName("wgs84Lat");
        NodeList lon = document.getElementsByTagName("wgs84Lon");
        ...
        for (int i = 0; i < nodelist.getLength(); i++) {
            ...
            result.setLatParam(lat.item(i).getChildNodes().item(0).getNodeValue());
            result.setLonParam(lon.item(i).getChildNodes().item(0).getNodeValue());
            results.add(result);
            log.info(String.format("No. #%d %s %s %s %s", i, result.getHosnameParam(), result.getCityParam(), result.getLatParam(), result.getLonParam()));
        }

//        for (int i = 0; i < addr.getLength(); i++) {
//            ConDTO result = new ConDTO();
//            results.add(result);
//            log.info(String.format("No test. #%d", i) + result.toString());
//        }
